...
class SQLiV2Scanner(BaseScanner):
    """Production-grade SQL Injection Scanner v2."""

    # Class-level constants (immutable)
    DBMS_ERRORS: typing.ClassVar[dict[str, tuple[str, ...]]] = {
        "MySQL": (
    ...
    TIME_PAYLOADS: typing.ClassVar[dict[str, tuple[str, ...]]] = {
        "MySQL": (
            "' AND SLEEP(5)--",
            '" AND SLEEP(5)--',
            "' OR SLEEP(5)--",
    ...
    def __init__(
        ...
        analyzer: ResponseAnalyzer | None = None,
    ) -> None:
        ...
        # Core components
        self._request_engine = request_engine or RequestEngine()
    ...
    def get_time_payloads(self, dbms: str | None = None) -> list[str]:
        """
        ...
        """
        if dbms is not None:
            return list(self.TIME_PAYLOADS.get(dbms, ()))
    ...
    def get_smart_payloads(self) -> list[str]:
        """
    ...
    def get_baseline_time(self) -> float:
        ...
        start = perf_counter()
        response = self._request_engine.get(self.target)
    ...
    def test_time_payload(self, payload: str) -> SQLFinding | None:
        ...
        response = self._request_engine.get(self.target, params={"q": payload})
        ...
        if not self.detect_time_sqli(baseline_time, injected_time):
            return None

        # Analyze Response
        analysis = self._analyzer.analyze(response, payload)
    ...
    def detect_dbms(self, response_text: str) -> str | None:
    ...
    def has_sql_error(self, response_text: str) -> bool:
        """Check whether the response contains a known SQL database error message."""
    ...
    def detect_boolean_sqli(self, normal_response: str, injected_response: str) -> bool:
        """Compare two responses and determine whether a Boolean-Based SQL Injection may exist."""
    ...
    def detect_time_sqli(
        ...
        """Detect a possible Time-Based SQL Injection by comparing response times."""
        if baseline_time <= 0 or injected_time <= 0:
            return False
    ...
    def calculate_confidence(
        ...
        *,
        ...
        """Calculate confidence score for the detected SQL Injection finding."""
        score = 0.0
    ...
    def get_boolean_payload_pair(self) -> tuple[str, str]:
    ...
    def compare_boolean_responses(self, true_response, false_response) -> bool:
        ...
        return self.detect_boolean_sqli(true_body, false_body)

    # ==========================================================
    # Response Evaluation
    # ==========================================================

    def evaluate_response(self, url: str, payload: str, response) -> SQLFinding | None:
        """
        ...
        Returns:
            SQLFinding if vulnerability detected, None otherwise
        """
        analysis = self._analyzer.analyze(response, payload)
        ...
        # Get severity from analyzer or default
        severity = "High"
        if analysis and hasattr(analysis, "risk") and hasattr(analysis.risk, "score"):
        ...
        # Build finding with all metadata
        finding = SQLFinding()
        finding.vulnerable = True
        finding.url = url
        finding.parameter = self._extract_parameter(url)
        finding.payload = payload
        finding.technique = "Error-Based"
        finding.dbms = dbms
        finding.analyzer = analysis

        # Set confidence in analyzer
        if analysis:
            analysis.risk.confidence = confidence

        # Add evidence
        finding.evidence.append(f"Detected database: {dbms}")
        finding.evidence.append(f"Confidence: {confidence:.2%}")
        finding.evidence.append(f"Payload: {payload}")

        # Populate metadata fields
        finding.title = f"SQL Injection Vulnerability in {finding.parameter} parameter"
        finding.description = (
            f"A SQL Injection vulnerability was detected in the '{finding.parameter}' parameter "
            f"using {finding.technique} technique. The backend database appears to be {dbms}."
        )
        finding.severity = severity
        finding.cwe = "CWE-89"
        finding.owasp = "OWASP Top 10 2021 - A03: Injection"
        finding.cvss = self._calculate_cvss_score(severity)
        finding.remediation = (
            "Use parameterized queries (Prepared Statements), "
            "avoid dynamic SQL construction, validate and sanitize user input, "
    ...
    def _calculate_cvss_score(self, severity: str) -> str:
        ...
        cvss_map = {
            "Critical": "CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:C/C:H/I:H/A:H (9.8)",
            "High": "CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H (7.5)",
            "Medium": "CVSS:3.1/AV:N/AC:L/PR:L/UI:N/S:U/C:L/I:L/A:L (5.0)",
            "Low": "CVSS:3.1/AV:N/AC:L/PR:H/UI:N/S:U/C:L/I:N/A:N (2.5)",
        }
        return cvss_map.get(
            severity, "CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H (7.5)"
        )
    ...
    def merge_findings(self, findings: list[SQLFinding]) -> list[SQLFinding]:
        """Merge duplicate SQL Injection findings."""
        merged: dict[tuple[str, str, str], SQLFinding] = {}

        for finding in findings:
            key = (finding.url, finding.payload, finding.dbms)
    ...
    def enrich_finding(self, finding: SQLFinding) -> SQLFinding:
        ...
        finding.title = (
            f"SQL Injection Vulnerability in {finding.parameter or 'unknown'} parameter"
        )
        finding.severity = severity
        finding.cwe = "CWE-89"
        finding.owasp = "OWASP Top 10 2021 - A03: Injection"
    ...
    def scan(self) -> list[SQLFinding]:
        ...
        # Time-Based
        for payload in self.get_time_payloads(self.detected_dbms):
            self.statistics["payloads"] += 1
    ...
    def test_payload(self, payload: str) -> SQLFinding | None:
        """Send one SQL Injection payload and evaluate the returned response."""
        self.requests_sent += 1
        ...
        finding = self.evaluate_response(self.target, payload, response)

        # Boolean-Based SQLi Verification
        true_payload, false_payload = self.get_boolean_payload_pair()

        true_response = self._request_engine.get(
            self.target, params={"q": true_payload}
        )
        false_response = self._request_engine.get(
            self.target, params={"q": false_payload}
        )

        if (
            true_response is not None
            and false_response is not None
            and self.validate_boolean_detection(true_response, false_response)
    ...
    def is_false_positive(self, true_response, false_response) -> bool:
        """Filter obvious false positives before reporting a Boolean-Based SQL Injection finding."""
        if true_response is None or false_response is None:
            return True
    ...
    def calculate_similarity(self, text1: str, text2: str) -> float:
        """Calculate similarity between two HTTP responses. Returns a value between 0.0 and 1.0."""
    ...
    def validate_boolean_detection(self, true_response, false_response) -> bool:
        """Validate Boolean-Based SQL Injection by comparing status codes, response sizes and similarity ratio."""
        if true_response is None or false_response is None:
            return False
        ...
        similarity = self.calculate_similarity(true_body, false_body)
        length_difference = abs(len(true_body) - len(false_body))
